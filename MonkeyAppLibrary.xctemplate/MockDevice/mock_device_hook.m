...
static int (*orig_syscall)(int code, ...);
...
static void* my_dlsym(void* handle, const char* symbol) {
    if (strcmp(symbol, "ptrace") == 0 ||
        strcmp(symbol, "_ptrace") == 0)
    {
        NSLog(@"[AntiAntiDebug] dlsym(\"%s\") intercepted", symbol);
        return my_ptrace;
    }
...
static int my_syscall(int code, ...) {
    va_list args;
    va_start(args, code);

    if (code == 26 /* SYS_ptrace */) {
        int request = va_arg(args, int);
        if (request == 31 /* PT_DENY_ATTACH */) {
            NSLog(@"[AntiAntiDebug] syscall(SYS_ptrace, PT_DENY_ATTACH) blocked");
            errno = EPERM;
            va_end(args);
            return -1;
        }
    ...
    int result = orig_syscall(code, args);
    va_end(args);
    return result;
}
...
static void init(void) {
    rebind_symbols((struct rebinding[16]) {
